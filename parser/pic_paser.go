...
func GeneratePICWarpHTML(picId, picPath string) string {
	...
	return fmt.Sprintf(`
<div id="%[1]s-wrapper" class="pic-viewer-wrapper" style="max-width:100%%;margin:20px auto;">
  <style>
    #%[1]s-wrapper {
      display: flex;
    ...
    #%[1]s-wrapper .pic-container:hover {
      transform: scale(1.02);
    }
    
    #%[1]s-wrapper .pic-image {
      max-width: 100%%;
      height: auto;
      display: block;
      border-radius: 8px;
    ...
    #%[1]s-wrapper .pic-image:hover {
      box-shadow: 0 8px 30px rgba(0, 0, 0, 0.15);
    }
    
    #%[1]s-wrapper .pic-loading {
      text-align: center;
      padding: 60px 20px;
      color: #7f8c8d;
      font-size: 14px;
    ...
    #%[1]s-wrapper .pic-spinner {
      border: 3px solid #ecf0f1;
      border-top: 3px solid #3498db;
      border-radius: 50%%;
      width: 40px;
      height: 40px;
      animation: pic-spin-%[1]s 1s linear infinite;
      margin: 0 auto 15px;
    }
    
    @keyframes pic-spin-%[1]s {
      0%% { transform: rotate(0deg); }
      100%% { transform: rotate(360deg); }
    }
    
    #%[1]s-wrapper .pic-error {
      text-align: center;
      padding: 60px 20px;
      color: #e74c3c;
      font-size: 14px;
  ...
  <div class="pic-container" id="%[1]s-container" style="display:none;">
    ...
    <div class="pic-toolbar">
      <button class="pic-btn" id="%[1]s-fullscreen-btn" title="Fullscreen View">
        🔍 View
      </button>
      <a href="%[2]s" download class="pic-btn" title="Download">
        ⬇ Download
      </a>
      <a href="%[2]s" target="_blank" rel="noopener noreferrer" class="pic-btn" title="Open in new tab">
        🔗 Open
      </a>
    </div>
  </div>
...
(function() {
  ...
  const elements = {
    ...
    container: document.getElementById(UNIQUE_ID + '-container'),
    image: document.getElementById(UNIQUE_ID + '-image'),
  ...
  // 初始化 Viewer.js
  function initViewer() {
    viewer = new Viewer(elements.image, {
      inline: false,
      button: true,
      navbar: false,
      title: true,
