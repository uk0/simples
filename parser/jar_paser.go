...
func GenerateJARPlayerHTML(playerID, jarPath, fileName, fileExt string) string {
  ...
  <style>
    ...
    #%[1]s-wrapper .mini-controls {
      display: flex;
      gap: 5px;
    }

    #%[1]s-wrapper .mini-btn {
      background: #333;
      color: #fff;
    ...
    #%[1]s-wrapper .mini-btn:hover {
      background: #4d94ff;
      border-color: #6ba3ff;
    }

    /* Phone Screen Frame */
    #%[1]s-wrapper .screen-frame {
      background: #1a1a1a;
      padding: 15px;
      position: relative;
    }
    ...
    #%[1]s-wrapper .key-map-item {
      display: flex;
      align-items: center;
      gap: 5px;
    }

    #%[1]s-wrapper .key-map-item .pc-key {
      background: #4d94ff;
      color: #fff;
      padding: 2px 6px;
      border-radius: 3px;
    ...
    #%[1]s-wrapper .key-map-item .arrow {
      color: #666;
    }

    #%[1]s-wrapper .key-map-item .phone-key {
      color: #ccc;
      font-size: 10px;
    }
  ...
  <div class="nokia-header">
    ...
    <div class="game-name">📱 %[3]s</div>
  </div>
  
  <!-- Control Bar -->
  <div class="control-bar">
    <div class="status-indicator">
      <span class="status-led" id="%[4]s-status-led"></span>
      <span id="%[4]s-status-text">Loading...</span>
    </div>
    <div class="mini-controls">
      <button class="mini-btn" onclick="location.reload()">↻</button>
      <a href="%[2]s" download class="mini-btn">⬇</a>
    </div>
  </div>
